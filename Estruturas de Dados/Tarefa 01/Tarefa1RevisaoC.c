...
/*void insereOrd(tipoNo **lst,int vl); tentei mas sem sucesso (função completa abaixo) */
...
/*void insereOrd(tipoNo **lst,int vl)
{
   ...
   if(  ( (*lst) != NULL ) && (vl > aux1->valor) )
	{
		aux2=aux1;
		aux1=aux1->prox;
		insereOrd(&(*lst)->prox,vl);
		aux2->prox=novoNo;
		novoNo->prox=aux1;
	}
